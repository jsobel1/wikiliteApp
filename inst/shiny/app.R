...
help_panel <- div(
  ...
  p("This app explores Wikipedia article revision histories: who wrote what and when, ",
    "how the article evolved, what citations were used, and signs of vandalism or ",
    "edit wars. The data backend is the ", tags$code("wikilite"), " R package; ",
    "the tab structure follows the original Microsoft History Flow study ",
    "(", tags$em("Viégas, Wattenberg & Dave, CHI 2004"), ")."),

  p("Pick a mode in the navigation bar:"),
  tags$ul(
    tags$li(tags$b("Single Article"), " — full multi-tab analysis of one article, ",
            "with controls in the left sidebar."),
    tags$li(tags$b("Corpus Analysis"), " — define one or more named, optionally ",
            "category-linked article lists and compare them across timeline, ",
            "citations and three networks."),
    tags$li(tags$b("Help"), " — this page.")
  ),

  # ────────────────────────────────────────────────────────────────────────
  h4("Single Article — sidebar parameters"),
  p("Found in the sticky panel on the left of the Single Article tab. ",
    "Click ", tags$b("Fetch →"), " to apply changes to the article ",
    "selection or date range; numeric/date parameters auto-debounce."),
  tags$ul(
    tags$li(tags$span(class = "help-param", "Article"),
            " — type to search Wikipedia (live suggestions via the ",
            tags$code("opensearch"), " API). Press Enter to keep a custom title."),
    ...
    tags$li(tags$span(class = "help-param", "Min |Δ| B"),
            ...
            "is below this threshold. Useful for hiding typo fixes."),
    tags$li(tags$span(class = "help-param", "Top N"),
            " — number of named editor groups kept distinct in the History Flow ",
            "chart. Smaller editors are merged into ", tags$em("Other"), "."),
    tags$li(tags$span(class = "help-param", "Sample size"),
            " — cap on how many revisions enter the analysis. Sampling happens ",
            "BEFORE the heavy ", tags$code("prepare_history()"), " step, so even ",
            "100 k-revision articles stay responsive."),
    tags$li(tags$span(class = "help-param", "Method"),
            " — how revisions are picked when sampling: ",
            tags$b("Most recent"), " (last N within the date range; default), ",
            tags$b("Even"), " (evenly spaced across the date range — best for ",
            "long-term overviews), ",
            tags$b("Random"), " (random subset, retaining first and last).")
  ),
  ...
  tags$ul(
    tags$li(tags$b("History Flow"),
            " — Sankey-like ribbon showing which editor groups own which slice of ",
            "the article over time. Per-editor swatches in the left rail let you ",
            "toggle visibility and recolour groups. Click a node to jump to that ",
            "revision in the Revision Inspector."),
    tags$li(tags$b("Citations"),
            " — fetches wikitext at N evenly spaced snapshots and extracts ",
            tags$code("<ref>"), " entries. Filters (Status, Reference type) drive ",
            "the timeline plot, the type-counts bar chart, the table and the ",
            "per-type XLSX export simultaneously. Click ",
            tags$b("Annotate DOIs (EuropePMC)"),
            " or ", tags$b("Annotate ISBNs (Google Books)"),
            " to enrich the table with journal / book metadata. ",
            tags$b("Compute Citation Latency"),
            " adds a publication-to-citation delay analysis (requires DOIs)."),
    tags$li(tags$b("Authorship"),
            " — anonymous vs. registered share over time, top editors, cumulative ",
            "edits, Gini coefficient, and the full editor table with revert rates."),
    tags$li(tags$b("Stability"),
            " — article size timeline, edit-size distribution, editing rhythm by ",
            "time-of-day / day-of-week. Includes a ", tags$b("SciScore over time"),
            " sub-section: per-snapshot journal % and DOI %, plus the citation ",
            "type mix per snapshot. Populated once you have run the Citations tab."),
    tags$li(tags$b("Vandalism & Wars"),
            " — heuristic detection of vandalism (rapid revert pairs), survival ",
            "time of vandal edits, and edit-war episodes (clusters of ",
            "revert / counter-revert)."),
    tags$li(tags$b("Revision Inspector"),
            " — pick any revision and see its wikitext coloured by token-level ",
            "authorship via the ", tags$b("WikiWho API"),
            " (",
            tags$a(href = "https://wikiwho-api.wmcloud.org/", target = "_blank",
                   rel = "noopener noreferrer", "wikiwho-api.wmcloud.org"),
            "). Falls back to a local line-diff heuristic if WikiWho is ",
            "unavailable. The diff panel shows added / removed lines vs. the ",
            "previous revision in the sample.")
  ),

  # ────────────────────────────────────────────────────────────────────────
  h4("Corpus Analysis"),
  p("A ", tags$b("corpus"), " is a named list of articles. Build as many as you ",
    "like and compare them across the right-hand tabs. The ",
    tags$b("Render comparison →"), " button at the bottom of the left sidebar ",
    "commits the current corpora to the panels on the right."),
  tags$ul(
    tags$li(tags$b("Adding a corpus"),
            " — type a name and paste article titles, one per line; or paste a ",
            "Wikipedia category name and click ", tags$b("Fetch category →"),
            " to auto-populate (the category name doubles as the corpus label)."),
    tags$li(tags$b("Timeline"),
            " — one marker per article positioned at its creation date, coloured ",
            "by corpus, sized by total revisions. Hover for creator, edit count, ",
            "pageviews and assessment. Click a marker to open the Wikipedia ",
            "article. Switch ", tags$em("Y-axis grouping"), " between corpus rows ",
            "and per-article rows."),
    tags$li(tags$b("Citations"),
            " — extracts ", tags$code("<ref>"),
            " citations from every corpus article (cached). Annotate DOIs / ISBNs ",
            "in batch, then explore: ",
            tags$ul(
              tags$li(tags$em("Reference composition"),
                      " — per-corpus stacked counts, or per-article stacks ",
                      "(counts or % normalised within each article)."),
              tags$li(tags$em("SciScore per corpus"),
                      " — grouped horizontal bars of journal % and DOI %."),
              tags$li(tags$em("Per-article SciScore"),
                      " — switch between a box plot per corpus (with article ",
                      "points and a corpus-mean diamond) and a sorted ",
                      "per-article bar chart on Journal % or DOI %."),
              tags$li(tags$em("Citation table"),
                      " — full DT table with article / corpus context, joined ",
                      "with EuropePMC and Google Books annotations when available.")
            )),
    tags$li(tags$b("Co-citation network"),
            " — articles sharing one or more DOIs, edge weight = #shared. ",
            "Coloured by corpus, sized by # DOIs. Click a node to open the ",
            "Wikipedia article."),
    tags$li(tags$b("Publication network"),
            " — bipartite Article ↔ Publication graph. Threshold sliders: ",
            tags$em("Min articles citing each DOI"), " and ",
            tags$em("Top N most-cited DOIs"), ". Click a publication node to ",
            "open the matching ", tags$code("doi.org"), " URL."),
    tags$li(tags$b("Wikilink network"),
            " — outgoing ", tags$code("[[wikilink]]"),
            " edges from corpus articles to other Wikipedia articles. ",
            "Sliders: ", tags$em("Min corpus articles linking to each target"),
            " and ", tags$em("Top N targets"), ". Toggle ",
            tags$em("Show only links between corpus articles"), " for an ",
            "internal network. Click any node to open Wikipedia.")
  ),
  ...
  tags$ul(
    tags$li(tags$b("Journal %"), " — share of citations classified as ",
            tags$em("Journal"), " (peer-reviewed). The classifier inspects the ",
            tags$code("{{cite …}}"), " template family in the wikitext."),
    tags$li(tags$b("DOI %"), " — share of citations that carry a DOI ",
            "(regardless of reference type).")
  ),
  p("In ", tags$b("Stability → SciScore over time"),
    " these are computed per snapshot (citations active at each snapshot). ",
    "In ", tags$b("Corpus → Citations"),
    " they are computed per article and per corpus, with cross-corpus comparison ",
    "via the box plot view."),

  # ────────────────────────────────────────────────────────────────────────
  h4("Caching, performance & robustness"),
  tags$ul(
    tags$li("All MediaWiki / WikiWho / xtools / EuropePMC / Google Books calls ",
            "use ", tags$code("httr"),
            " with a descriptive User-Agent and exponential backoff on 429 / 5xx ",
            "responses (the default base R ", tags$code("url()"),
            " path is throttled aggressively)."),
    tags$li("Per-session caches: raw history (on disk via ",
            tags$code("tools::R_user_dir()"), "); article info, latest wikitext, ",
            "DOI annotations, ISBN annotations (in memory). Re-rendering or ",
            "re-annotating after a parameter change is therefore near-instant."),
    tags$li("EuropePMC annotations use batched ",
            tags$code("OR"), "-queries (~25 DOIs per request, ~25× faster than ",
            "the per-DOI fallback)."),
    tags$li("Errors in heavy observers (annotation runs, citation extraction, ",
            "revision loads) are caught and surfaced as toast notifications — ",
            "the session keeps running."),
    tags$li("Clickable nodes / markers (Timeline, all three corpus networks) ",
            "open the matching Wikipedia article (or ", tags$code("doi.org"),
            ") in a new tab.")
  ),
  ...
  h4("Exports"),
  tags$ul(
    tags$li(tags$b("Single Article — Citations"), ": citations XLSX (full + ",
            "per-type), BibTeX (requires ", tags$code("rcrossref"), ")."),
    tags$li(tags$b("Single Article — Authorship / Stability / Vandalism"),
            ": editor breakdown, revision history, vandalism events, and ",
            "edit-war episodes XLSX. SciScore-over-time XLSX in Stability."),
    tags$li(tags$b("Corpus — Citations"), ": citations XLSX, per-corpus ",
            "SciScore XLSX, per-article SciScore XLSX, all-SciScore multi-sheet ",
            "workbook (corpus / article / cross-corpus stats / counts by type), ",
            "annotated DOIs XLSX (EuropePMC + 'where used in corpus'), and ",
            "annotated ISBNs XLSX (Google Books + 'where used in corpus').")
  ),

  # ────────────────────────────────────────────────────────────────────────
  h4("Optional packages"),
  tags$ul(
    tags$li(tags$code("DT"), " — interactive sortable / filterable tables."),
    tags$li(tags$code("openxlsx"), " — every XLSX export."),
    tags$li(tags$code("visNetwork"), " — all corpus network plots."),
    tags$li(tags$code("rcrossref"), " — BibTeX export."),
    tags$li(tags$code("europepmc"), " — fallback for the batched EuropePMC ",
            "fetcher when ", tags$code("httr"), " isn't available."),
    tags$li(tags$code("httr"), " — recommended; required for batched + cached ",
            "EuropePMC, throttled Google Books, and the WikiWho client.")
  ),

  # ────────────────────────────────────────────────────────────────────────
  h4("Data sources"),
  tags$ul(
    tags$li(tags$a(href = "https://www.mediawiki.org/wiki/API:Main_page",
                   target = "_blank", rel = "noopener noreferrer",
                   "MediaWiki Action API"),
            " — revision history, wikitext, opensearch, category members."),
    tags$li(tags$a(href = "https://wikiwho-api.wmcloud.org/",
                   target = "_blank", rel = "noopener noreferrer",
                   "WikiWho"),
            " — token-level authorship for the Revision Inspector."),
    tags$li(tags$a(href = "https://xtools.wmcloud.org/",
                   target = "_blank", rel = "noopener noreferrer",
                   "xtools"),
            " — article metadata for the corpus timeline (creator, ",
            "creation date, editor count, pageviews, assessment)."),
    tags$li(tags$a(href = "https://europepmc.org/RestfulWebService",
                   target = "_blank", rel = "noopener noreferrer",
                   "EuropePMC REST API"),
            " — DOI metadata + citation latency."),
    tags$li(tags$a(href = "https://developers.google.com/books",
                   target = "_blank", rel = "noopener noreferrer",
                   "Google Books API"),
            " — ISBN metadata.")
  ),
  ...
  tags$blockquote(
    tags$em("Viégas, F. B., Wattenberg, M., & Dave, K. (2004). ",
            "Studying cooperation and conflict between authors with history ",
            "flow visualizations. Proceedings of CHI 2004, 575–582. "),
    tags$a(href = "https://doi.org/10.1145/985692.985765",
           target = "_blank", rel = "noopener noreferrer",
